...
#Set this to your Kron-cli program
cli = "Kron-cli"
...
#Set this information in your Kron.conf file (in datadir, not testnet6)
rpc_user = 'rpcuser'
rpc_pass = 'rpcpass555'
...
def audit(filter):
    ...
    for asset, properties in assets.items():
        ...
        if (total_issued == total_for_asset):
            ...
            log_failure(msg)
            send_notification(notification_emails, "Kron Asset Audit Failed", msg)
            #exit(-1)
        ...
        if len(assets) == count:
            ...
            if (send_alerts_on_success and audits_failed == 0):
              send_notification(notification_emails, "Kron Asset Audit Success", "All " + str(len(assets)) + " assets audited.")
