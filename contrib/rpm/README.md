...
The RPM spec file provided here is for Kron-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://Kron.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Kron git repository](https://github.com/KronProject/Kron) but are not part of
...
Currently only `Source10` is used, for the example `Kron.conf` file.
...
in the Kron git repository. At present only an SVG version of the Kron
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the Kron
...
needed when 0.12.1 is released, a proper fix is already in the Kron git
master branch.
...
idea to build Kron Core against a static version of that library compiled
...
The `Kron.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `Kron-server` package. At this point, most RPM based
...
The `Kron-server` package should have SELinux support. How to properly do
...
Kron-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/Kron/). He has been
packaging Kron for Fedora at least since 2012.
...
4. I renamed the `Kron` package that contains the Qt GUI to `Kron-core` as
that appears to be how the general population refers to it, in contrast to
`Kron-xt` or `Kron-classic`. I wanted to make sure the general population
