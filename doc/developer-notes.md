...
The Kron Doxygen documentation can be generated by running `make docs` from the top-level 
...
Run with the -testnet option to run with "play Krons" on the test network, if you
...
Kron Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- KronMiner : Generates Krons (if wallet is enabled).
...
However, in open source software such as Kron Core, where everyone uses
...
pay attention to for reviewers of Kron Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Kron Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Kron Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Kron Core so that they can be integrated
...
- src/libsecp256k1
  - Upstream at https://github.com/Kron-core/secp256k1/ ; actively maintaned by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/Kron-core/ctaes ; actively maintained by Core contributors.
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Kron/Kron.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `Kron-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `Kron-cli` and the GUI debug console use this table to determine how to
