...
std::string HelpMessageCli()
{
    ...
    strUsage += HelpMessageOpt("-rpcwallet=<walletname>", _("Send RPC for non-default wallet on RPC server (argument is wallet filename in Krond directory, required if Krond/-Qt runs with multiple wallets)"));
        ...
        if (!GetAuthCookie(&strRPCUserColonPass)) {
            throw std::runtime_error(strprintf(
                _("Could not locate RPC credentials. No authentication cookie could be found, and RPC password is not set.  See -rpcpassword and -stdinrpcpass.  Configuration file: (%s)"),
                    GetConfigFile(gArgs.GetArg("-conf", Kron_CONF_FILENAME)).string().c_str()));
                ...
                if (!error.isNull()) {
                    ...
                    if (error.isObject())
                    {
                        ...
                        if (errCode.isNum() && errCode.get_int() == RPC_WALLET_NOT_SPECIFIED) {
                            strPrint += "\nTry adding \"-rpcwallet=<filename>\" option to Kron-cli command line.";
                        }
                    }
                } else {
                    // Result
