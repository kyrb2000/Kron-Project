...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for ESS
    ...
    if (p_rewardSnapshot.strDistributionAsset != "ESS") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is ESS with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using ESS differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "ESS") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Kron address (already validated during ownership snapshot creation)
