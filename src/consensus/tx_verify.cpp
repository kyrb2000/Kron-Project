...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs, bool fMempoolCheck, bool fBlockCheck)
{
    ...
    for (const auto& txout : tx.vout)
    {
        ...
        /** ESS START */
        // Find and handle all new OP_ESS_ASSET null data transactions
    ...
    if (tx.IsNewAsset()) {
        ...
        // above transaction types.  Also fail if it contains OP_ESS_ASSET opcode but wasn't a valid script.
        for (auto out : tx.vout) {
            int nType;
            bool _isOwner;
            if (out.scriptPubKey.IsAssetScript(nType, _isOwner)) {
                ...
                if (out.scriptPubKey.Find(OP_ESS_ASSET)) {
                    if (out.scriptPubKey[0] != OP_ESS_ASSET) {
                        return state.DoS(100, false, REJECT_INVALID,
