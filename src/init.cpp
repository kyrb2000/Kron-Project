 ...
 * between Krond, and Kron-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    FlushWallets();
#endif
    GenerateKrons(false, 0, GetParams());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), Kron_CONF_FILENAME));
    if (mode == HMM_KronD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), Kron_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/KronProject/Kron>";
    const std::string URL_WEBSITE = "<https://Kron.org>";
...
/** Sanity checks
 *  Ensure that Kron is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Kron process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", Kron_CONF_FILENAME)).string());
    ...
    GenerateKrons(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
