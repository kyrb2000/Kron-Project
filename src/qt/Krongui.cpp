...
const std::string KronGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString KronGUI::DEFAULT_WALLET = "~Default";

KronGUI::KronGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void KronGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Kron address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and Kron: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to Kron addresses"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Kron addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Kron addresses"));
    ...
    openAction->setStatusTip(tr("Open a Kron: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Kron command-line options").arg(tr(PACKAGE_NAME)));
...
void KronGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/Krontext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* KronLabelLayout = new QVBoxLayout(toolbarWidget);
        KronLabelLayout->addWidget(label);
        KronLabelLayout->addWidget(toolbar);
        KronLabelLayout->setDirection(QBoxLayout::TopToBottom);
        KronLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Kron Market Price"));
        ...
        labelVersionUpdate->setText("<a href=\"https://github.com/KronProject/Kron/releases\">New Wallet Version Available</a>");
        ...
        // Get the latest Kron release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/KronProject/Kron/releases";
...
void KronGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool KronGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool KronGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void KronGUI::setWalletActionsEnabled(bool enabled)
{
...
void KronGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void KronGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void KronGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void KronGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void KronGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void KronGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void KronGUI::openClicked()
{
...
void KronGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void KronGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void KronGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void KronGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void KronGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void KronGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** Kron START */
void KronGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void KronGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void KronGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void KronGUI::gotoRestrictedAssetsPage()
{
...
void KronGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Kron network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void KronGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void KronGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void KronGUI::updateHeadersSyncProgressLabel()
{
...
void KronGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void KronGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Kron"); // default title
...
void KronGUI::changeEvent(QEvent *e)
{
...
void KronGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void KronGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void KronGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "Kron")
        msg += tr("Amount: %1\n").arg(KronUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(KronUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void KronGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void KronGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool KronGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool KronGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void KronGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void KronGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void KronGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void KronGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void KronGUI::detectShutdown()
{
...
void KronGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void KronGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void KronGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(KronGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static bool ThreadSafeMnemonic(KronGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void KronGUI::subscribeToCoreSignals()
{
...
void KronGUI::unsubscribeFromCoreSignals()
{
...
void KronGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void KronGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<KronUnits::Unit> units = KronUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const KronUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(KronUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(KronUnits::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (KronUnits::Unit u : KronUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(KronUnits::name(u)), this);
...
{
    setText(KronUnits::name(newUnits));
}
...
void KronGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=ESSBTC"));
    networkManager->get(*request);
}

#ifdef ENABLE_WALLET
void KronGUI::mnemonic()
{
...
void KronGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/KronProject/Kron/releases"));
