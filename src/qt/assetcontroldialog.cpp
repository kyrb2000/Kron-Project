    ...
    platformStyle(_platformStyle)
{
    ...
    // see https://github.com/KronProject/Kron/issues/5716
...
{
    GUIUtil::setClipboard(KronUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void AssetControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = KronUnits::Kron;
    ...
    l2->setText(KronUnits::formatWithCustomName(QString::fromStdString(strAssetName), nAssetAmount));        // Amount
    l3->setText(KronUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(KronUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(KronUnits::formatWithCustomName(QString::fromStdString(strAssetName), nChange));        // Change
    if (nPayFee > 0)
    {
    ...
    for (const std::pair<QString, std::vector<COutput>> &coins : mapAssetCoins) {
        ...
        for (const COutput &out : coins.second) {
            ...
            if (ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show Kron address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, KronUnits::format(nDisplayUnit, nAmount));
        ...
        if (treeMode) {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, KronUnits::format(nDisplayUnit, nSum));
