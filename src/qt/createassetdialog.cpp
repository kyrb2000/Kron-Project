...
void CreateAssetDialog::setUpValues()
{
    ...
    QStringList list;
    list.append(tr("Main Asset") + " (" + KronUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::ROOT)) + ")");
    list.append(tr("Sub Asset") + " (" + KronUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::SUB)) + ")");
    list.append(tr("Unique Asset") + " (" + KronUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::UNIQUE)) + ")");
    list.append(tr("Messaging Channel Asset") + " (" + KronUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::MSGCHANNEL)) + ")");
    list.append(tr("Qualifier Asset") + " (" + KronUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::QUALIFIER)) + ")");
    list.append(tr("Sub Qualifier Asset") + " (" + KronUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::SUB_QUALIFIER)) + ")");
    list.append(tr("Restricted Asset") + " (" + KronUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::RESTRICTED)) + ")");
                                 ...
                                 const CAmount& watchBalance, const CAmount& watchUnconfirmedBalance, const CAmount& watchImmatureBalance)
{
    ...
    {
        ui->labelBalance->setText(KronUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balance));
    }
}
...
void CreateAssetDialog::CheckFormState()
{
    ...
    if (!(ui->addressText->text().isEmpty() || IsValidDestination(dest)) && assetNameValid) {
        ...
        showMessage(tr("Warning: Invalid Kron address"));
        return;
    }
...
void CreateAssetDialog::onCreateAssetClicked()
{
    ...
    QString amount = "<b>" + QString::fromStdString(ValueFromAmountString(GetBurnAmount(type), 8)) + " Kron";
    amount.append("</b>");
    ...
    if(nFeeRequired > 0)
    {
        ...
        questionString.append(KronUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), nFeeRequired));
    ...
    for (KronUnits::Unit u : KronUnits::availableUnits())
    {
        if(u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(KronUnits::formatHtmlWithUnit(u, totalAmount));
    }
    questionString.append(tr("Total Amount %1")
                                  .arg(KronUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount)));
...
void CreateAssetDialog::updateSmartFeeLabel()
{
    ...
    ui->labelSmartFee->setText(KronUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), feeRate.GetFeePerK()) + "/kB");
...
void CreateAssetDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Kron address"));
        }
        else // Valid address
        {
...
void CreateAssetDialog::updateFeeMinimizedLabel()
{
    ...
    if (ui->radioSmartFee->isChecked())
        ...
        ui->labelFeeMinimized->setText(KronUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value()) + "/kB");
    }
}
...
void CreateAssetDialog::updateMinFeeLabel()
{
    if (model && model->getOptionsModel())
        ui->checkBoxMinimumFee->setText(tr("Pay only the required fee of %1").arg(
                KronUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetRequiredFee(1000)) + "/kB")
        );
}
