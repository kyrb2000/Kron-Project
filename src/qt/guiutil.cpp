...
#include "Kronaddressvalidator.h"
#include "Kronunits.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Kron address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new KronAddressEntryValidator(parent));
    widget->setCheckValidator(new KronAddressCheckValidator(parent));
}
...
bool parseKronURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no Kron: URI
    if(!uri.isValid() || uri.scheme() != QString("Kron"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!KronUnits::parse(KronUnits::Kron, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseKronURI(QString uri, SendCoinsRecipient *out)
{
    // Convert Kron:// to Kron:
    //
    //    Cannot handle this later, because Kron:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Kron://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "Kron:");
    }
    QUrl uriInstance(uri);
    return parseKronURI(uriInstance, out);
}

QString formatKronURI(const SendCoinsRecipient &info)
{
    QString ret = QString("Kron:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(KronUnits::format(KronUnits::Kron, info.amount, false, KronUnits::separatorNever));
        paramCount++;
    }
...
bool openKronConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(Kron_CONF_FILENAME);
    ...
    /* Open Kron.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Kron.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Kron (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Kron (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "Kron.desktop";
    return GetAutostartDir() / strprintf("Kron-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a Kron.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Kron\n";
        else
            optionFile << strprintf("Name=Kron (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the Kron app
...
bool GetStartOnSystemStartup()
{
    CFURLRef KronAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (KronAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, KronAppUrl);

    CFRelease(KronAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Kron app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, KronAppUrl, nullptr, nullptr);
    }
