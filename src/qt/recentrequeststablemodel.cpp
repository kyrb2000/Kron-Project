    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return KronUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, KronUnits::separatorNever);
            else
                return KronUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+KronUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
