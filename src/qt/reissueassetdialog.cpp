...
//        ui->optInRBF->setCheckState(model->getDefaultWalletRbf() ? Qt::Checked : Qt::Unchecked);
        ...
        ui->reissueCostLabel->setText(tr("Cost") + ": " + KronUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::REISSUE)));
...
void ReissueAssetDialog::CheckFormState()
{
    ...
    if (!ui->addressText->text().isEmpty()) {
        if (!IsValidDestination(dest)) {
            showMessage(tr("Invalid Kron Destination Address"));
            return;
        }
    }
...
void ReissueAssetDialog::onReissueAssetClicked()
{
    ...
    QString amount = "<b>" + QString::fromStdString(ValueFromAmountString(GetReissueAssetBurnAmount(), 8)) + " Kron";
    amount.append("</b>");
