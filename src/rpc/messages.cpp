...
{
    return AreMessagesDeployed() ? "" : "\nTHIS COMMAND IS NOT YET ACTIVE!\nhttps://github.com/KronProject/rips/blob/master/rip-0005.mediawiki\n";
}
...
UniValue viewallmessages(const JSONRPCRequest& request) {
    ...
    if (!fMessaging) {
        ...
        ret.push_back("Messaging is disabled. To enable messaging, run the wallet without -disablemessaging or remove disablemessaging from your Kron.conf");
        return ret;
    }
...
UniValue subscribetochannel(const JSONRPCRequest& request) {
    ...
    if (!fMessaging) {
        throw JSONRPCError(RPC_DATABASE_ERROR, "Messaging is disabled. To enable messaging, run the wallet without -disablemessaging or remove disablemessaging from your Kron.conf");
    }
