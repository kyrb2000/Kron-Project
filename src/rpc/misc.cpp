...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total Kron balance of the wallet\n"
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given Kron address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The Kron address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The Kron address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Kron address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are Kron addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) Kron address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The Kron address to use for the signature.\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "Kron-cli and the GUI. There is no server-side difference."
        );
...
{
    if (type == 2) {
        address = CKronAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CKronAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
{
    if (params[0].isStr()) {
        CKronAddress address(params[0].get_str());
        uint160 hashBytes;
        int type = 0;
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CKronAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  {\n"
            "    \"address\"  (string) The base58check encoded address\n"
            "    \"assetName\"  (string) The name of the associated asset (ESS for Kron)\n"
    ...
    if (includeAssets) {
        ...
        if (!mempool.getAddressIndex(addresses, ESS, indexes)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
        }
    }
...
UniValue getaddressutxos(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"assetName\"   (string, optional) Get UTXOs for a particular asset instead of ESS ('*' for all assets).\n"
            "}\n"
            "\nResult\n"
            "[\n"
            "  {\n"
            "    \"address\"  (string) The address base58check encoded\n"
            "    \"assetName\" (string) The asset associated with the UTXOs (ESS for Kron)\n"
...
UniValue getaddressdeltas(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1 || !request.params[0].isObject())
        throw std::runtime_error(
            ...
            "  \"assetName\"   (string, optional) Get deltas for a particular asset instead of ESS.\n"
            "}\n"
            "\nResult:\n"
            "[\n"
            "  {\n"
            "    \"assetName\"  (string) The asset associated with the deltas (ESS for Kron)\n"
            "    \"satoshis\"  (number) The difference of satoshis\n"
            "    \"txid\"  (string) The related txid\n"
            "    \"index\"  (number) The related input or output index\n"
            "    \"height\"  (number) The block height\n"
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"assetName\"  (string) The asset associated with the balance (ESS for Kron)\n"
            "    \"balance\"  (string) The current balance in satoshis\n"
            "    \"received\"  (string) The total number of satoshis received (including change)\n"
            "  },...\n"
            "\n]"
    ...
    if (includeAssets) {
        ...
        for (std::vector<std::pair<uint160, int> >::iterator it = addresses.begin(); it != addresses.end(); it++) {
            if (!GetAddressIndex((*it).first, (*it).second, ESS, addressIndex)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
            }
        }
...
UniValue getaddresstxids(const JSONRPCRequest& request)
{
    ...
    for (std::vector<std::pair<uint160, int> >::iterator it = addresses.begin(); it != addresses.end(); it++) {
        if (includeAssets) {
            ...
            if (start > 0 && end > 0) {
                if (!GetAddressIndex((*it).first, (*it).second, ESS, addressIndex, start, end)) {
                    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
                }
            } else {
                if (!GetAddressIndex((*it).first, (*it).second, ESS, addressIndex)) {
