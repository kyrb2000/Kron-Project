// Copyright (c) 2019-2020 The Kron Core developers
...
UniValue distributereward(const JSONRPCRequest& request) {
    if (request.fHelp || request.params.size() < 4)
        throw std::runtime_error(
                ...
                "3. \"distribution_asset_name\"    (string, required) The name of the asset that will be distributed, or ESS\n"
                ...
                "\nExamples:\n"
                + HelpExampleCli("distributereward", "\"TRONCO\" 12345 \"ESS\" 1000")
                ...
                + HelpExampleRpc("distributereward", "\"PHATSTACKS\" 34987 \"ESS\" 100000 \"mwN7xC3yomYdvJuVXkVC7ymY9wNBjWNduD,n4Rf18edydDaRBh7t6gHUbuByLbWEoWUTg\"")
        );
    ...
    CAmount distribution_amount = AmountFromValue(request.params[3], (distribution_asset_name == "ESS"));
    ...
    if (request.params.size() > 5) {
        ...
        if (!change_address.empty() && !IsValidDestinationString(change_address))
            throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("Invalid change address: Use a valid ESS address"));
    }
    ...
    if (distribution_asset_name != "ESS") {
...
UniValue getdistributestatus(const JSONRPCRequest& request) {
    if (request.fHelp || request.params.size() < 4)
        throw std::runtime_error(
                ...
                "\nExamples:\n"
                + HelpExampleCli("getdistributestatus", "\"TRONCO\" 12345 \"ESS\" 1000")
                ...
                + HelpExampleRpc("getdistributestatus", "\"PHATSTACKS\" 34987 \"ESS\" 100000 \"mwN7xC3yomYdvJuVXkVC7ymY9wNBjWNduD,n4Rf18edydDaRBh7t6gHUbuByLbWEoWUTg\"")
        );
