...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** ESS START */
    case OP_ESS_ASSET              : return "OP_ESS_ASSET";
    /** ESS END */
...
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_ESS_ASSET) { // OP_ESS_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == ESS_R) { // Check to see if ESS starts at 27 ( this->size() < 105)
                if ((*this)[28] == ESS_V)
                    if ((*this)[29] == ESS_N)
                        index = 30;
            } else {
                if ((*this)[28] == ESS_R) // Check to see if ESS starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == ESS_V)
                        if ((*this)[30] == ESS_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == ESS_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == ESS_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == ESS_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == ESS_R) {
...
{
    // 1 OP_ESS_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_ESS_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_ESS_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_ESS_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_ESS_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
