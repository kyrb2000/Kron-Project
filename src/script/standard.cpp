...
bool Solver(const CScript& scriptPubKey, txnouttype& typeRet, std::vector<std::vector<unsigned char> >& vSolutionsRet)
{
    ...
    if (mTemplates.empty())
    {
        ...
        // Kron address tx, sender provides hash of pubkey, receiver provides signature and pubkey
    ...
    if (scriptPubKey.size() >= 1 && scriptPubKey[0] == OP_ESS_ASSET && scriptPubKey.IsPushOnly(scriptPubKey.begin()+1)) {
...
namespace
{
    ...
    public:
        ...
        bool operator()(const CKeyID &keyID) const {
            script->clear();
            *script << OP_ESS_ASSET << ToByteVector(keyID);
            return true;
        }

        bool operator()(const CScriptID &scriptID) const {
            script->clear();
            *script << OP_ESS_ASSET << ToByteVector(scriptID);
            return true;
        }
    };
} // namespace
