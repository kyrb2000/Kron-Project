...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of Kron NOT allowed
        BOOST_CHECK(!IsAssetNameValid("ESS", type));
        BOOST_CHECK(!IsAssetNameValid("Kron", type));
        BOOST_CHECK(!IsAssetNameValid("Kron", type));

        //- Versions of Kron ALLOWED
        BOOST_CHECK(IsAssetNameValid("Kron.COIN", type));
        BOOST_CHECK(IsAssetNameValid("Kron_COIN", type));
        BOOST_CHECK(IsAssetNameValid("ESSSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERESS", type));
        BOOST_CHECK(IsAssetNameValid("KronSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEKron", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_KronS", type));
        BOOST_CHECK(IsAssetNameValid("SEESSOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("Kron", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("Kron", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
