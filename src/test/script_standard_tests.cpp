...
BOOST_FIXTURE_TEST_SUITE(script_standard_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(script_standard_solver_failure_test)
    {
        ...
        s.clear();
        s << OP_ESS_ASSET << std::vector<unsigned char>({75}) << OP_ADD;
        BOOST_CHECK(!Solver(s, whichType, solutions));
    ...
    BOOST_AUTO_TEST_CASE(script_standard_extractdestination_test)
    {
        ...
        s.clear();
        s << OP_ESS_ASSET << std::vector<unsigned char>({75});
        BOOST_CHECK(!ExtractDestination(s, address));
    ...
    BOOST_AUTO_TEST_CASE(script_standard_ismine_test)
    {
        ...
        // OP_ESS_ASSET at front of script
        {
            CBasicKeyStore keystore;
            keystore.AddKey(keys[0]);

            scriptPubKey.clear();
            scriptPubKey << OP_ESS_ASSET << ToByteVector(pubkeys[0]);
