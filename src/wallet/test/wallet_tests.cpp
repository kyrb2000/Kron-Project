...
BOOST_FIXTURE_TEST_SUITE(wallet_tests, WalletTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(coin_selection_test)
    {
        ...
        for (int i = 0; i < RUN_TESTS; i++)
        {
            ...
            BOOST_CHECK_EQUAL(nValueRet, 1 * COIN);  // we should get 1 ESS in 1 coin
            BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);

            BOOST_CHECK(testWallet.SelectCoinsMinConf(195 * CENT, 1, 1, 0, vCoins, setCoinsRet, nValueRet));
            BOOST_CHECK_EQUAL(nValueRet, 2 * COIN);  // we should get 2 ESS in 1 coin
            BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);
        ...
        // after.
        {
            ...
            BOOST_CHECK_EQUAL(response.write(),
                              strprintf("[{\"success\":false,\"error\":{\"code\":-1,\"message\":\"Rescan failed for key with creation "
                                        ...
                                        "by pruning or data corruption (see Krond log for details) and could be dealt with by "
