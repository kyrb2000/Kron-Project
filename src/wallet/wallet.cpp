...
void CWallet::AvailableCoinsAll(std::vector<COutput>& vCoins, std::map<std::string, std::vector<COutput> >& mapAssetCoins, bool fGetESS, bool fGetAssets, bool fOnlySafe, const CCoinControl *coinControl, const CAmount& nMinimumAmount, const CAmount& nMaximumAmount, const CAmount& nMinimumSumAmount, const uint64_t& nMaximumCount, const int& nMinDepth, const int& nMaxDepth) const {
    vCoins.clear();

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (fGetESS) { // Looking for ESS Tx OutPoints Only
                    if (fESSLimitHit) // We hit our limit
                        continue;

                    // We only want ESS OutPoints. Don't include Asset OutPoints
                    if (isAssetScript)
                        continue;
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListAssets() const
{
    ...
    // https://github.com/KronProject/Kron/pull/10340, but that change has been
    // postponed until after https://github.com/KronProject/Kron/pull/10244 to
                                   ...
                                   const AssetType& assetType, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-Kron-address
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    /** ESS START */
                    // Check to see if you need to make an asset data outpoint OP_ESS_ASSET data
                    ...
                    if (IsDust(txout, ::dustRelayFee) && !IsScriptTransferAsset(recipient.scriptPubKey)) /** ESS START */ /** ESS END */
                    {
 ...
 * https://Krontalk.org/?topic=54527, or
 * https://github.com/KronProject/Kron/pull/1393.
 */
