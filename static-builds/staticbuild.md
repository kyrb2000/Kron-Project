...
sudo find ${DISTNAME}/bin -type f -executable -exec ../Kron/contrib/devtools/split-debug.sh {} {} {}.dbg \;
sudo find ${DISTNAME}/lib -type f -exec ../Kron/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make dist
SOURCEDIST=`echo Kron-*.tar.gz`
mkdir -p temp
cd temp
tar xf ../$SOURCEDIST
find Kron-* | sort | tar --no-recursion --mode='u+rw,go+r-w,a+X' --owner=0 --group=0 -c -T - | gzip -9n > ../$SOURCEDIST
cd ~/Kron
...
cp -f Kron-*setup*.exe ~/release/unsigned/
mkdir -p ~/win64
...
cd ~/
sudo rm -rf win64
cp -rf Kron/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir unsigned
mv ~/Kron/Kron-*setup-unsigned.exe unsigned/
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/Kron-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer Kron-*-win*-unsigned.tar.gz to ~/win64/  
...
#transfer Kron-*-win*-setup.exe back to the Ubuntu18 build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
cd ~/
sudo rm -rf win32
cp -rf Kron/contrib/windeploy ~/sign/win32
...
#transfer Kron-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
~/Kron/contrib/macdeploy/extract-osx-sdk.sh
...
git clone https://github.com/Kronproject/Kron
mkdir ~/Kron/depends/SDKs
#transfer MacOSX10.11.sdk.tar.gz to the folder Kron/depends/SDKs
cd Kron/depends/SDKs && tar -xf MacOSX10.11.sdk.tar.gz 
...
cd ~/Kron
make deploy
$PWD/depends/x86_64-apple-darwin14/native/bin/dmg dmg "Kron-Core.dmg" ~/release/unsigned/$DISTNAME-osx-unsigned.dmg
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/Kron-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer Kron-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
