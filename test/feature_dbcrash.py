...
class ChainstateWriteCrashTest(KronTestFramework):
    ...
    def restart_node(self, node_index, expected_tip):
        ...
        while time.time() - time_start < 120:
            ...
            try:
                ...
                # If Krond exits, then try again.  wait_for_node_exit()
                # should raise an exception if Krond doesn't exit.
                self.log.debug("Wait for node exit ~~ during restart, node: %s", node_index)
                self.wait_for_node_exit(node_index, timeout=10)
            self.crashed_on_restart += 1
            time.sleep(1)

        # If we got here, Krond isn't coming back up on restart.  Could be a
        # bug in Krond, or we've gotten unlucky with our dbcrash ratio --
