...
Also test that nodes that send unsupported service bits to Krond are disconnected
and don't receive a VERACK. Unsupported service bits are currently 1 << 5 and
1 << 7 (until August 1st 2018).

UPDATE: Kron RIP-2 uses bit 1 << 5.  Currently there are no unsupported service bits.
"""
...
class CNodeNoVersionBan(CLazyNode):
    ...
    # NOTE: implementation-specific check here. Remove if Krond ban behavior changes
    def on_open(self, conn):
        super().on_open(conn)
...
class P2PLeakTest(KronTestFramework):
