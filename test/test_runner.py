...
`test/functional/test_framework/test_framework.py:KronTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/Kron_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    logging.debug("Temporary test directory at %s" % tmpdir)

    # Don't run tests on Windows by default
    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/KronProject/Kron/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/KronProject/Kron/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    # Check that the build was configured with wallet, utils, and Krond
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_Krond = config["components"].getboolean("ENABLE_KronD")
    if not (enable_wallet and enable_cli and enable_Krond):
        print("No functional tests to run. Wallet, utils, and Krond must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if Krond is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "Krond"]) is not None:
            print("%sWARNING!%s There is already a Krond process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "KronD" not in os.environ:
        os.environ["KronD"] = build_dir + '/src/Krond' + exeext
        os.environ["KronCLI"] = build_dir + '/src/Kron-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `Kron-cli help` (`rpc_interface.txt`).
